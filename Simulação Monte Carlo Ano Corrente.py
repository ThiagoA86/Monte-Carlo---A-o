...
data = pd.DataFrame()
ano_corrente = pd.DataFrame()
#Colentando os dados pelo yahoo finance o fechamento ajustado
data[tickers] = pdr.DataReader(tickers,data_source='yahoo',start=startdate, end=fim)['Adj Close']
ano_corrente[tickers] = pdr.DataReader(tickers,data_source='yahoo',start=fim)['Adj Close']
print(ano_corrente.size)
...
#Z
#Intervalo de Pregoes do ano sessões B3 e 3 simulaçoes de previsoes Monte Carlo
t_intervalo = ano_corrente.size
interacao = 3
...
# Simulação para Pregoes do ano sessões B3 e 3 simulaçoes
...
#Transformando o ano em Array Float64 e Cacontenar com a lista de preco
ano_corrente= ano_corrente.to_numpy(dtype=np.float64)
lista_preco=np.concatenate((lista_preco, ano_corrente), axis=1)
...
plt.plot(ano_corrente)
plt.title('Simulação da '+tickers)
plt.xlabel("Dias", fontsize=14)
plt.xticks(range(0,(ano_corrente.size+10),10),rotation=45)
...
plt.plot(lista_preco[:,-1],label=f'Preço Real',linestyle='dashed',linewidth=3)
...
fig.add_trace(go.Scatter(y=lista_preco[:,-1],name='Preço Real',line_width=4,line_dash='10 5'))
fig.update_xaxes(title_text='Dias',range=[0,(ano_corrente.size+10)],dtick=10)
